#![allow(static_mut_refs)]
use sails_rs::{
    collections::HashSet,
    gstd::{msg, service},
    prelude::*,
};
mod funcs;
use crate::services;
use core::str::FromStr;
use vft_service::{Service as VftService, Storage};

#[derive(Default)]
pub struct ExtendedStorage {
    minters: HashSet<ActorId>,
    burners: HashSet<ActorId>,
    admins: HashSet<ActorId>,
}

static mut EXTENDED_STORAGE: Option<ExtendedStorage> = None;

#[event]
#[derive(Encode, Decode, TypeInfo)]
#[codec(crate = sails_rs::scale_codec)]
#[scale_info(crate = sails_rs::scale_info)]
pub enum Event {
    Minted { to: String, value: String },
    Burned { from: String, value: String },
}
#[derive(Clone)]
pub struct ExtendedService {
    vft: VftService,
}

impl ExtendedService {
    pub fn create() -> Self {
        Self {
            vft: VftService::new(),
        }
    }

    pub fn init(name: String, symbol: String, decimals: u16) -> Self {
        let admin = msg::source();
        unsafe {
            EXTENDED_STORAGE = Some(ExtendedStorage {
                admins: [admin].into(),
                minters: [admin].into(),
                burners: [admin].into(),
            });
        };
        ExtendedService {
            vft: <VftService>::init(name, symbol, decimals),
        }
    }

    pub fn get_mut(&mut self) -> &'static mut ExtendedStorage {
        unsafe {
            EXTENDED_STORAGE
                .as_mut()
                .expect("Extended vft is not initialized")
        }
    }
    pub fn get(&self) -> &'static ExtendedStorage {
        unsafe {
            EXTENDED_STORAGE
                .as_ref()
                .expect("Extended vft is not initialized")
        }
    }
}

impl From<ExtendedService> for VftService {
    fn from(value: ExtendedService) -> Self {
        value.vft
    }
}

#[service(extends = VftService, events = Event)]
impl ExtendedService {
    #[export]
    pub fn mint(&mut self, to: String, value: String) -> bool {
        sails_rs::gstd::debug!("MINT MSG SOURCE {:?}", msg::source());
        // if !self.get().minters.contains(&msg::source()) {
        //     panic!("Not allowed to mint")
        // };

        let to_id = parse_actor(&to);
        let value_u256 = parse_u256(&value);
        let mutated = services::utils::panicking(|| {
            funcs::mint(Storage::balances(), Storage::total_supply(), to_id, value_u256)
        });
        if mutated {
            self.emit_event(Event::Minted { to, value })
                .expect("Notification Error");
        }
        mutated
    }

    #[export]
    pub fn burn(&mut self, from: String, value: String) -> bool {
        if !self.get().burners.contains(&msg::source()) {
            panic!("Not allowed to burn")
        };
        let from_id = parse_actor(&from);
        let value_u256 = parse_u256(&value);
        let mutated = services::utils::panicking(|| {
            funcs::burn(Storage::balances(), Storage::total_supply(), from_id, value_u256)
        });
        if mutated {
            self.emit_event(Event::Burned { from, value })
                .expect("Notification Error");
        }
        mutated
    }

    #[export]
    pub fn grant_admin_role(&mut self, to: String) {
        self.ensure_is_admin();
        self.get_mut().admins.insert(parse_actor(&to));
    }

    #[export]
    pub fn grant_minter_role(&mut self, to: String) {
        self.ensure_is_admin();
        self.get_mut().minters.insert(parse_actor(&to));
    }

    #[export]
    pub fn grant_burner_role(&mut self, to: String) {
        self.ensure_is_admin();
        self.get_mut().burners.insert(parse_actor(&to));
    }

    #[export]
    pub fn revoke_admin_role(&mut self, from: String) {
        self.ensure_is_admin();
        self.get_mut().admins.remove(&parse_actor(&from));
    }

    #[export]
    pub fn revoke_minter_role(&mut self, from: String) {
        self.ensure_is_admin();
        self.get_mut().minters.remove(&parse_actor(&from));
    }

    #[export]
    pub fn revoke_burner_role(&mut self, from: String) {
        self.ensure_is_admin();
        self.get_mut().burners.remove(&parse_actor(&from));
    }

    #[export]
    pub fn minters(&self) -> Vec<String> {
        self.get().minters.clone().iter().map(|id| id.to_string()).collect()
    }

    #[export]
    pub fn burners(&self) -> Vec<String> {
        self.get().burners.clone().iter().map(|id| id.to_string()).collect()
    }

    #[export]
    pub fn admins(&self) -> Vec<String> {
        self.get().admins.clone().iter().map(|id| id.to_string()).collect()
    }
}

impl ExtendedService {
    fn ensure_is_admin(&self) {
        if !self.get().admins.contains(&msg::source()) {
            panic!("Not admin")
        };
    }
}

fn parse_actor(id: &str) -> ActorId {
    ActorId::from_str(id).expect("Unable to decode to ActorId")
}

fn parse_u256(amount: &str) -> U256 {
    U256::from_dec_str(amount).unwrap_or_else(|_| panic!("Invalid U256 decimal string"))
}